...
mod tests {
    ...
    fn test_merge_pdfs() {
        ...
        assert!(merged_doc.is_ok(), "Merged PDF should be loadable: {:?}", merged_doc.err());
        ...
        assert!(pages.len() >= 2, "Merged PDF should have at least 2 pages, got {}", pages.len());
    ...
    fn test_merge_same_pdf_multiple_times() {
        ...
        assert!(result.is_ok(), "Merge with duplicates should succeed: {:?}", result.err());
        assert!(Path::new(output).exists(), "Output file should be created");
        ...
        assert!(pages.len() >= 3, "Merged PDF should have at least 3 pages, got {}", pages.len());
    ...
    fn test_merge_nonexistent_file() {
        ...
        assert!(result.is_err(), "Merge should fail with nonexistent input file");
        
        // Output file should not be created
        assert!(!Path::new(output).exists(), "Output file should not be created on failure");
    }
    ...
    fn test_merge_empty_input() {
        ...
        if Path::new(output).exists() {
            ...
            if let Ok(doc) = merged_doc {
                let pages = doc.get_pages();
                assert_eq!(pages.len(), 0, "Merged PDF from empty input should have 0 pages");
            }
