...
pub fn delete_pages(input: &str, output: &str, pages_to_delete: &[u32]) -> Result<()> {
    let doc = Document::load(input)
        .with_context(|| format!("Failed to load PDF '{}'", input))?;
    ...
    for &page_num in pages_to_delete {
        if page_num == 0 || page_num > total_pages as u32 {
            return Err(anyhow::anyhow!(
                ...
                page_num, total_pages, total_pages
            ));
        }
    }
    
    // Create a set for faster lookup of pages to delete (convert to 0-based indexing)
    let delete_set: std::collections::HashSet<usize> = pages_to_delete
        .iter()
        .map(|&p| (p - 1) as usize)
        .collect();
...
mod tests {
    ...
    fn test_delete_pages() {
        ...
        assert!(result.is_ok(), "Delete pages should succeed: {:?}", result.err());
    ...
    fn test_delete_invalid_page() {
        ...
        assert!(result.is_err(), "Delete should fail for invalid page number");
    ...
    fn test_delete_all_pages() {
        ...
        assert!(result.is_err(), "Delete should fail when trying to delete all pages");
        assert!(!Path::new(output).exists(), "Output file should not be created on failure");
    }
}